            ...
            <div className="max-w-7xl mx-auto bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[var(--radius-2xl)] shadow-[var(--shadow-sm)] px-4 sm:px-6 py-4">
                <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                        <button
                            // Full navigation: client-side routing (both
                            // router.push and <Link>) is inert on the player
                            // page — verified in a headless browser. Using the
                            // router here made this button do nothing at all.
                            onClick={() => { window.location.href = isPremium ? '/premium' : '/'; }}
                    ...
                    <div className="flex items-center gap-3">
                        {/* Plain anchor, not next/link: <Link> intercepts the
                            click and routes client-side, which is inert on this
                            page (same reason as the home button above). */}
                        <a
